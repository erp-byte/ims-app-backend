...
def lookup_sku(item_description: str, company: Company, db: Session) -> dict | None:
    ...
    row = db.execute(
        text(f"""
            SELECT id, item_description, material_type, item_category, sub_category, sale_group
            FROM {tables['sku']}
...
def sku_dropdown(
    ...
    if item_description:
        row = db.execute(
            text(f"""
                SELECT material_type, item_category, sub_category, sale_group FROM {tbl}
...
def sku_global_search(
    ...
    rows = db.execute(
        text(f"""
            SELECT DISTINCT id, item_description, material_type, item_category, sub_category, sale_group
            FROM {tbl}
            WHERE {where_sql}
            ORDER BY item_description ASC
            LIMIT :limit OFFSET :offset
