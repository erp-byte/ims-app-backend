...
def _map_line_row(row) -> dict:
    return {
        ...
        "package_size": str(row.package_size) if row.package_size is not None else "0",
    ...
    rows = db.execute(
        text("""
            SELECT id, request_id, rm_pm_fg_type, item_category, sub_category,
                   ...
                   package_size, net_weight, total_weight, lot_number,
...
def create_request(data: RequestCreate, created_by: str, db: Session) -> dict:
    ...
    for line in data.article_data:
        ...
        package_size_val = int(float(line.package_size)) if line.package_size else 0

        # Use frontend-calculated net_weight if available, otherwise compute
        frontend_nw = float(line.net_weight) if line.net_weight else 0
        if frontend_nw > 0:
            net_weight = frontend_nw
        elif line.material_type.upper() == "FG" and package_size_val:
            net_weight = package_size_val * pack_size_f * qty_i
        else:
        ...
        row = db.execute(
            ...
            """),
            {
                ...
                "quantity": qty_i,
                "uom": line.uom,
                "package_size": line.package_size or "0",
    ...
    rows = db.execute(
        text("""
            SELECT id, header_id, rm_pm_fg_type, item_category, sub_category,
                   item_desc_raw, pack_size, qty, uom,
                   package_size, net_weight, total_weight, batch_number, lot_number,
                   created_at, updated_at
...
def create_transfer(data: TransferCreate, created_by: str, db: Session) -> dict:
    ...
    for line in data.lines:
        ...
        if line.material_type.upper() == "FG" and package_size_val:
            net_weight = package_size_val * pack_size_f * qty_i
        else:
            net_weight = pack_size_f * qty_i
    ...
    # Update line weights from actual scanned box data
    if boxes:
        total_net = sum(float(b.net_weight or 0) for b in boxes)
        total_gross = sum(float(b.gross_weight or 0) for b in boxes)
        for line_row in lines:
            db.execute(
                text("""
                    ...
                    WHERE id = :lid
                """),
                {"nw": total_net, "tw": total_gross, "lid": line_row.id},
            )
...
def delete_transfer(transfer_id: int, db: Session) -> dict:
    ...
    if existing.status in ("Received", "Completed", "Dispatch"):
        raise HTTPException(
            400,
