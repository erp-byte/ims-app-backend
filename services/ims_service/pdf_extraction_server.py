...
def _ensure_tables(db: Session):
    ...
    """))
    # Add challan_no column if table already exists without it
    try:
...
def submit_material_out(
    ...
    """), {
        ...
        "purpose_of_work": header.get("purpose_of_work", ""),
    ...
    for item in line_items:
        ...
        db.execute(text("""
            INSERT INTO jb_materialout_lines
                (header_id, sl_no, item_description, material_type, item_category, sub_category,
                 hsn_sac, gst_rate, quantity_kgs, quantity_boxes, rate_per_kg, amount,
                 uom, case_pack, unit_case_pack, net_weight, total_weight,
        ...
        """), {
            ...
            "unit_case_pack": str(item.get("unit_case_pack", "")),
        ...
        "items": [
            {
                ...
                "uom": safe(l, "uom", "KG"),
                "unit_case_pack": str(safe(l, "unit_case_pack", "")),
                ...
                "hsn_sac": safe(l, "hsn_sac"),
                "gst_rate": safe(l, "gst_rate"),
...
def get_material_out_by_challan(
    ...
    lines = db.execute(text("""
        SELECT sl_no, item_description, sub_category, uom, unit_case_pack,
               quantity_boxes, net_weight, total_weight, lot_number, line_remarks,
               hsn_sac, gst_rate, rate_per_kg, amount, material_type, item_category,
        ...
        "items": [
            {
                ...
                "sub_category": l[2] or "",
                "uom": l[3] or "KG",
                "unit_case_pack": l[4] or "",
                "quantity_boxes": int(l[5] or 0),
                "net_weight": float(l[6] or 0),
                "total_weight": float(l[7] or 0),
                "lot_number": l[8] or "",
                "remarks": l[9] or "",
                "hsn_sac": l[10] or "08041020",
                "gst_rate": l[11] or "0%",
                "rate_per_kg": float(l[12] or 0),
                "amount": float(l[13] or 0),
                "material_type": l[14] or "",
                "item_category": l[15] or "",
                "batch_number": l[16] or "",
                "box_id": l[20] or "",
                "transaction_no": l[21] or "",
            }
            for l in lines
        ],
    }
...
def submit_material_in(
    ...
    ir_result = db.execute(text("""
        INSERT INTO jb_work_inward_receipt
            (ir_number, challan_no, header_id, receipt_date, receipt_type, vehicle_no, driver_name, remarks, created_by)
        VALUES
            (:ir_number, :challan_no, :header_id, :receipt_date, :receipt_type, :vehicle_no, :driver_name, :remarks, :created_by)
        RETURNING id
    """), {
        ...
        "header_id": header_id,
        "receipt_date": payload.get("received_date", ""),
...
def delete_job_work_record(
    ...
    # Delete inward lines first (via cascade from inward_receipt), then lines, then header
    db.execute(text("""
        ...
        "line_items": [
            {
                ...
                "hsn_sac": l[5] or "",
                "gst_rate": l[6] or "",
                "quantity_kgs": float(l[7] or 0),
                "quantity_boxes": int(l[8] or 0),
                "rate_per_kg": float(l[9] or 0),
                "amount": float(l[10] or 0),
                "uom": l[11] or "",
                "case_pack": str(l[12] or ""),
                "unit_case_pack": str(l[13] or ""),
                "net_weight": str(l[14] or ""),
                "total_weight": str(l[15] or ""),
                "batch_number": l[16] or "",
                "lot_number": l[17] or "",
                "manufacturing_date": l[18] or "",
                "expiry_date": l[19] or "",
                "line_remarks": l[20] or "",
            }
            for l in lines
        ],
    }
