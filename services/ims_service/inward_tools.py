...
def create_inward_bulk_sticker(payload, db: Session) -> dict:
    ...
    # 2) Bulk insert articles (strip box-specific fields)
    ...
    if payload.articles:
        ...
        for a in payload.articles:
            ...
            d = clean_date_fields(a.model_dump())

            d.pop("box_count", None)
    ...
    for article in payload.articles:
        ...
        for box_num in range(1, article.box_count + 1):
            ...
            box_infos.append(GeneratedBoxInfo(
                ...
                gross_weight=float(article.box_gross_weight) if article.box_gross_weight is not None else None,
...
def update_inward(
    company: Company, transaction_no: str, payload, db: Session
) -> dict:
    tables = table_names(company)
    ...
    tx_new = clean_date_fields(payload.transaction.model_dump(exclude_none=True))
    tx_new.pop("transaction_no", None)
    old_tx = dict(existing._mapping)
    tx_changes = {}
    for field, new_val in tx_new.items():
        old_val = old_tx.get(field)
        # Normalise Decimal / float for comparison
    ...
    if tx_changes:
        set_parts = [f"{k} = :{k}" for k in tx_changes]
        tx_changes["txno"] = transaction_no
        db.execute(
            text(f"UPDATE {tables['tx']} SET {', '.join(set_parts)} WHERE transaction_no = :txno"),
            tx_changes,
        )
            ...
            if existing:
                ...
                db.execute(
                    ...
                    text(f"""
                        ...
                        SET net_weight = COALESCE(:net_weight, net_weight), gross_weight = COALESCE(:gross_weight, gross_weight),

                            lot_number = COALESCE(:lot_number, lot_number), count = COALESCE(:count, count)
            ...
            db.execute(
                ...
                text(f"""
                    ...
                    SET net_weight = COALESCE(:net_weight, net_weight), gross_weight = COALESCE(:gross_weight, gross_weight),
                        ...
                        lot_number = COALESCE(:lot_number, lot_number), count = COALESCE(:count, count), box_id = :box_id
